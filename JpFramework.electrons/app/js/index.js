...
$(function(){
    ...
    window.bgIndex=0;
    // function test() {
    //     Hander("Test!Test?test1=测试测试&test2=123456&test3=34&test4=false", function (error, result) {
    //         alert(result);
    //     });
    // }
    function padLeft(h){
    ...
    function initCenter(){
        ...
        function init(count){
            ...
            if(count%3600000==0||window.weather==null){
                window.weather=$.ajax({url:"http://wthrcdn.etouch.cn/weather_mini?city=兰州市",async:false});
                window.weather=JSON.parse(window.weather.responseText);
        ...
        initPoetry();
        function initPoetry(){
            var data=$.ajax({url:"http://api.apiopen.top/recommendPoetry",async:false});
